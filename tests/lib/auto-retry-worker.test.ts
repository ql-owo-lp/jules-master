...
describe('startAutoRetryWorker', () => {
  ...
  it('should not run if auto-retry is disabled', async () => {
    ...
    const dbThenMock = vi.fn().mockImplementation((callback) => {
      callback([{ autoRetryEnabled: false }]);
  ...
  it('should send retry message to failed sessions', async () => {
    ...
    const settingsThenMock = vi.fn().mockImplementation((callback) => {
      callback([{ autoRetryEnabled: true, autoRetryMessage: 'Retry?' }]);
    ...
    vi.mocked(actions.getSession)
      .mockResolvedValueOnce({ id: '1', state: 'FAILED', updateTime: new Date().toISOString() } as any)
