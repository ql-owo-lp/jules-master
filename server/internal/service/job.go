...
func (s *JobServer) ListJobs(ctx context.Context, _ *emptypb.Empty) (*pb.ListJobsResponse, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
            ...
            &j.RequirePlanApproval, &cronJobId, &profileId,
		); err != nil {
        ...
        if cronJobId.Valid { j.CronJobId = cronJobId.String }
        if profileId.Valid { j.ProfileId = profileId.String }
...
func (s *JobServer) GetJob(ctx context.Context, req *pb.GetJobRequest) (*pb.Job, error) {
    if req.Id == "" { return nil, fmt.Errorf("id required") }
...
func (s *JobServer) CreateJob(ctx context.Context, req *pb.CreateJobRequest) (*pb.Job, error) {
    ...
    if req.SessionIds == nil { sessionIdsJSON = []byte("[]") }
...
func (s *JobServer) CreateManyJobs(ctx context.Context, req *pb.CreateManyJobsRequest) (*emptypb.Empty, error) {
    ...
    for _, j := range req.Jobs {
         ...
         if j.SessionIds == nil { sessionIdsJSON = []byte("[]") }
         
         automationModeStr := "AUTOMATION_MODE_UNSPECIFIED"
         if j.AutomationMode == pb.AutomationMode_AUTO_CREATE_PR { automationModeStr = "AUTO_CREATE_PR" }
         
         id := j.Id; if id == "" { id = uuid.New().String() }
         createdAt := j.CreatedAt; if createdAt == "" { createdAt = time.Now().Format(time.RFC3339) }
...
func (s *JobServer) UpdateJob(ctx context.Context, req *pb.UpdateJobRequest) (*emptypb.Empty, error) {
    ...
    if !updates { return &emptypb.Empty{}, nil }
